...
const ViewgleAnalytics = require("./analytics");
...
define = opts => {
  ...
  connectLogAndExit = () => {
    ...
    let saveData = new ViewgleAnalytics(state.data);
  ...
  collectData = req => {
    ...
    if (req.originalUrl.indexOf("/viewgle/logRoute") != -1) {
      req.originalUrl = req.originalUrl.replace("/viewgle/logRoute", "");
    }
...
router.get("/viewgle/getAllEntries", (req, res) => {
  mongoose.connect(state.mongoConnection, { useNewUrlParser: true });
  ViewgleAnalytics.find({}, (err, resp) => {
    if (err)
      console.log(
    ...
    ViewgleAnalytics.find({ date: { $gte: monthStart, $lt: today } })
      .sort({ date: "asc" })
      .exec((err, resp) => {
        if (err)
          console.log(
        ...
        ViewgleAnalytics.find({ date: { $gte: weekStart, $lt: today } })
          .sort({ date: "asc" })
          .exec((err, resp) => {
            if (err)
              console.log(
